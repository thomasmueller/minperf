 ...
 */
public class XorFilter_nbit implements Filter {
    ...
    public static XorFilter_nbit construct(long[] keys, int bitsPerKey) {
        return new XorFilter_nbit(keys, bitsPerKey);
    }

    public XorFilter_nbit(int size, int bitsPerFingerprint, int hashIndex, BitArray fingerprints) {
        this.size = size;
        if (bitsPerFingerprint > 10 && bitsPerFingerprint != 12 && bitsPerFingerprint != 16) {
     ...
     */
    public XorFilter_nbit(long[] keys, int bitsPerFingerprint) {
        ...
        fingerprints = new BitArray(m, bitsPerFingerprint);
        for (int i = 0; i < fp.length; i++) {
            fingerprints.put2(i, fp[i]);
        }
    }
    ...
    public boolean mayContain(long key) {
        ...
        f ^= fingerprints.get2(h0);
        f ^= fingerprints.get2(h1);
        f ^= fingerprints.get2(h2);
    ...
    public static XorFilter_nbit read(BitBuffer source) {
        ...
        BitArray fingerprints = new BitArray(arrayLength, bitsPerFingerprint);
        ...
        for (int i = 0, j = 0; i < arrayLength; i++) {
            if (set.get(i)) {
                long x = fp[j++];
                fingerprints.put2(i, (int) x);
            } else {
                fingerprints.put2(i, 0);
            }
        }
        return new XorFilter_nbit(size, bitsPerFingerprint, hashIndex, fingerprints);
    }
